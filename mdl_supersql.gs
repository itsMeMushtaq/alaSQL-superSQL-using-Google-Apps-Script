 ...
 * @customFunction
 * SUPERSQL(sqlAsString,dataAsArray,...dataAsArrays)
 * @param {"SELECT * FROM ?"} sqlAsString SQL like statetment.
 * @param {A1:G20} dataAsArray Range with first row as unique headers.
 */
function SUPERSQL(sqlAsString,dataAsArray,...dataAsArrays) {
  ...
  let sqlData = [arrayToObjectOfArrays(dataAsArray)];
